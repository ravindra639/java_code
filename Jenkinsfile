      ...
      stage ('Deploy')
      {
          steps {
           bat '''
				...
				if "%check_condition%" == "false" (powerhsell (Get-Content '$WORKSPACE\\space folder\\main.py') -replace 'sfdcSandbox = True', 'sfdcSandbox = False' | Out-File -encoding ASCII '$WORKSPACE\\space folder\\main.py')
				else
